...
import { Modes, modesExcludeFromCheck, Names } from 'backend/models/Games';

import { removeLocalStorageValue } from 'helpers/local_storage';

const Bracket = dynamic(() => import('./components/bracket'));
...
export const Tournament: FC = () => {
  ...
  const [information, setInformation] = useRecoilState(
    selector_tournamentInformation,
  );

  const setIsTournamentStart = useSetRecoilState(selector_isTournamentStart);

  const [isRegularMode, setIsRegularMode] = useRecoilState(
    selector_isRegularTournament,
  );
  ...
  const [isShowFormCheckUser, setIsShowFormCheckUser] =
  ...
  useEffect(() => {
    if (mode) {
      setIsShowFormCheckUser(!modesExcludeFromCheck.includes(mode));
    }
  }, [mode]);
  ...
  const getButtons = (props: FormikProps<Values>) => (
    <>
      ...
      <Button
        disabled={!props.isValid || !isValidateUser}
        type="submit"
        variant="contained">
        Начать турнир
      </Button>
  ...
  const isShowInformation = useMemo(
    () => !!(isCheckTournament && mode),
    [mode, isCheckTournament],
  );
            ...
            {open ? (
              <div className={cn(styles.information, styles.container)}>
                ...
                {isShowFormCheckUser ? (
                  ...
                  <Button
                    variant="contained"
                    onClick={handleStartRegularTournament}>
                    Начать турнир
                  </Button>
                )}
              </div>
